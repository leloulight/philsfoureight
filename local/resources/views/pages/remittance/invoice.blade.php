...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="#">Remittance Service</a></li>
...
{!! Form::open(array('url' => '/remittance/submit', 'method' => 'post', 'onsubmit' => 'submit.disabled = true; return true;')) !!}
<input type="hidden" id="accountno" name="accountno" value="{{$request['accountno']}}">
<input type="hidden" id="memberid" name="memberid" value="{{$request['memberid']}}">
<input type="hidden" id="amount" name="amount" value="{{$request['amount']}}">
<input type="hidden" id="fee" name="fee" value="{{$request['fee']}}">
<input type="hidden" id="note" name="note" value="{{$request['note']}}">
  ...
  <div class="row invoice-info">
    ...
    <div class="col-sm-4 invoice-col">
      To:
      <address>
        <strong>{{$receiver[0]->firstname}} {{$receiver[0]->middlename}} {{$receiver[0]->lastname}} {{$receiver[0]->suffix}}</strong><br>
        {{$receiver[0]->street_address}}<br>
        {{$receiver[0]->city_name}}, {{$receiver[0]->province_name}}<br>
        {{$receiver[0]->email}}<br>
        {{$receiver[0]->mobileno}}
      </address>
    </div>
    <!-- /.col -->
    <div class="col-sm-4 invoice-col">
      <b>Remittance Date:</b> {{date("m/d/Y")}}<br>
  ...
  <div class="row">
    <div class="col-xs-12 table-responsive">
      <table class="table table-striped">
        ...
        <tbody>
          <tr>
            <td>Remittance Service</td>
            <td>{{$receiver[0]->accountno}} - {{$receiver[0]->username}}</td>
            <td>
    ...
    <div class="col-xs-6">
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <tr>
            <th>Fee:</th>
            <td>Php {{number_format($request['fee'], 2)}}</td>
          </tr>
          <tr>
            <th>Total:</th>
            <td>Php {{number_format(($request['amount'] + $request['fee']), 2)}}</td>
          </tr>
        </table>
      </div>
    </div>
